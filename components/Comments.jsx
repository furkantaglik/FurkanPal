        ...
        <div className="lg:w-1/2 w-full rounded-lg p-3 grid bg-gray-900 justify-center  mt-10">
            <div className="grid grid-cols-4 justify-between gap-6">
                <div className="col-span-3">
                    <h1 className="text-lg  font-bold mb-3">Yorumlar {comments?.length}</h1>
                    {message && (
                    ...
                    <textarea name="comment" value={newComment} onChange={(e) => setNewComment(e.target.value)}
                        className="w-full p-3 rounded-lg focus:outline-black bg-gray-600 focus:bg-transparent"
                        rows={4} placeholder="Yorumunuzu Buraya Yazın Küfür Hakaret Spoler İçeren Yorumlar Silinir ">
                    </textarea>
                </div>
            ...
            <div>
                {
                    comments?.map((comment, index) => (
                        <div className="mb-10 border-b" key={index}>
                            <h4 className=" text-teal-500 font-bold">Anonim <span className="text-slate-400 ms-4 font-normal"> 20.3.2022</span></h4>
                            <div className="mt-3 p-2 font-mono">
                                {comment.commentText}
                            </div>
                        </div>
                    ))
