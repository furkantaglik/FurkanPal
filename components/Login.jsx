...
export default function Login() {
  ...
  return (
    <div className='grid grid-cols-1 mx-auto w-96 bg-neutral-800 text-center font-bold my-10 gap-y-5 border border-gray-600 rounded-lg p-5'>
      ...
      <input type='email' name="email" className=' p-1  bg-transparent border'
        onChange={(e) => setEmail(e.target.value)}
        value={email} autoComplete="on"
      />

      <label htmlFor="password" className='text-start'>Şifre</label>
      <input type="password" name="password" className=' p-1 bg-transparent border'
      ...
      {change ?
        <button type="button" className='text-white bg-red-600 hover:bg-red-700 rounded-lg p-2' onClick={handleSignUp}>Kayıt Ol</button>
        :
        <button type="button" className='text-white bg-red-600 hover:bg-red-700 rounded-lg p-2' onClick={handleSignIn}>Giriş Yap</button>
      }
