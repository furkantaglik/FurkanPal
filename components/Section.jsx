    ...
    return (
        <section className="max-w-screen-xl mx-auto px-2 my-10">
            <div className='flex justify-between items-center text-center mb-10'>
                ...
                <span className='flex justify-center items-center gap-2 cursor-pointer hover:text-red-600' >Tümü <FaRegArrowAltCircleRight /> </span>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8 md:gap-12">
                {movies.map((movie) => (
                    <div className="group overflow-hidden rounded-lg h-96 w-full  relative transition-transform duration-200 ease-in-out hover:z-10 hover:transition-transform hover:scale-110" key={movie.id}>
                        <Link href={`/Watch/${movie.id}`}>
                            ...
                            <div className='relative md:opacity-0 group-hover:opacity-100 transition duration-200 w-ful h-full'>
                                ...
                                <span className='absolute bottom-0 left-0 p-2 font-semibold'>{movie.title}</span>
                                <span className='absolute right-0 bottom-0 text-center text-red-500 font-bold bg-black p-2 rounded-lg'>{movie.imdb_score || "?"}</span>
                            </div>
                        </Link>
                    </div>
                ))}
