import { collection, doc, getDocs, updateDoc, addDoc, deleteDoc, Timestamp, query, where } from "firebase/firestore";
...
export async function getSearchResults(searchText, setMessage) {
    try {
        ...
        for (const res of fsData) {
            if (searchText.length>2 &&res.title.trim().toLocaleLowerCase('tr').includes(searchText.trim().toLocaleLowerCase('tr'))) {
                data.push(res)
            }
        }
        return data
